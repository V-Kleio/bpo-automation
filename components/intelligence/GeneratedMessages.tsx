    ...
    <Tabs value={step} onValueChange={(v) => setStep(v as "1" | "2" | "3" | "4")}>
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="text-right">
          <div className="text-xs font-semibold text-zinc-700">{info.label}</div>
          <div className="text-[11px] text-zinc-500">{info.subline}</div>
        </div>
      </div>
                ...
                return (
                  ...
                  >
                    <div className="mb-3 flex items-center gap-2">
                      <div className="flex h-7 w-7 items-center justify-center rounded-full bg-zinc-900 text-[10px] font-semibold text-white">
                        {st.name
                          .split(" ")
                          .filter(Boolean)
                          .map((n) => n[0])
